...
const roomForView = () => view.startsWith('room:')?view.slice(5):view.startsWith('agent:')?state.agents[view.slice(6)]?.direct_room:'agent_chat';
...
async function go(next){rememberScroll();if(next.startsWith('room:')||next.startsWith('agent:')){if(!tabs.includes(next))tabs.push(next);saveTabs();}view=next;oldest=null;render();await renderContent();await markCurrentRoomRead();}
function tabName(key){return key.startsWith('room:')?'# '+(state.rooms[key.slice(5)]?.name||'chat'):'@'+(state.agents[key.slice(6)]?.handle||'agent');}
...
function additionalChannelNav(){
  return Object.values(state.rooms)
    ...
    .map(room=>button(
      'nav',
      `<span class="mono">#</span><span class="nav-text">${esc(room.name||room.id)}</span>${unreadRoom(room.id)?`<span class="notification-badge">${unreadRoom(room.id)}</span>`:''}`,
      'nav-item '+(view===`room:${room.id}`?'selected':''),
      `data-view="room:${esc(room.id)}" title="${esc(room.name||room.id)}"`,
    )).join('');
}
function render(){
  if(!state)return;
...
async function renderContent(){
  if(!state)return;const stream=$('#stream');if(!stream)return;const thisView=view;
  ...
  if(view==='activity'){
    ...
    const result=await api('history?project='+project+'&room='+encodeURIComponent(r.id));if(view!==thisView)return;
    stream.innerHTML=(result.messages.length===50?button('older','Load earlier messages','ghost small',`data-before="${result.messages[0].seq}" data-room="${r.id}"`):'')+(result.messages.length?result.messages.map(m=>message(m)).join(''):empty('The conversation starts here','Share a brief, ask a question, or mention an agent.'))+Object.values(state.votes).filter(v=>v.room===r.id).map(voteCard).join('')+typingHTML(r.id);composer(r.id);stream.scrollTop=stream.scrollHeight;
  }else if(view==='votes'){
    ...
    header('Agents','Independent sessions. Durable identities.','agents',button('new-agent',icon('plus')+' Add profile','small'));
    ...
    stream.innerHTML=`<div class="agent-header">${avatar(a.id,'large')}<div><h2 style="color:${esc(a.color)}">${esc(a.handle)} ${state.config.lead_agent_id===a.id?'<span class="tag lead">LEAD</span>':''}</h2><p class="muted">${esc(a.role)} · ${esc(a.provider)}</p><div class="uuid">${a.id} ${button('copy-id',icon('copy'),'ghost small',`data-id="${a.id}" title="Copy UUID" aria-label="Copy UUID"`)}</div></div></div><div class="agent-controls">${button('rename-agent','Rename','small',`data-id="${a.id}"`)}${button('toggle-agent',icon(a.paused?'play':'pause')+' '+(a.paused?'Resume':'Pause at checkpoint'),'small',`data-id="${a.id}"`)}${state.config.lead_agent_id!==a.id?button('appoint-lead','Designate lead','small',`data-id="${a.id}"`):''}${button('agent-connect','Connect terminal','ghost small',`data-id="${a.id}"`)}<span class="tag">${esc(presence(a))}</span></div><div class="segment">${button('agent-pane','Direct chat',selectedAgentPane==='chat'?'active':'','data-pane="chat"')}${button('agent-pane','Working notes',selectedAgentPane==='notes'?'active':'','data-pane="notes"')}${button('agent-pane','Checkpoint & context',selectedAgentPane==='checkpoint'?'active':'','data-pane="checkpoint"')}</div><div id="agent-content"></div>`;
    if(selectedAgentPane==='chat'){const res=await api('history?project='+project+'&room='+a.direct_room);if(view!==thisView)return;$('#agent-content').innerHTML=(res.messages.map(m=>message(m)).join('')||empty('A direct conversation','Messages here reach this agent.'))+typingHTML(a.direct_room);composer(a.direct_room);}
    else if(selectedAgentPane==='notes'){const res=await api('history?project='+project+'&agent_id='+a.id);if(view!==thisView)return;$('#agent-content').innerHTML=res.messages.map(m=>`<div class="notes-block"><div class="eyebrow">WORKING NOTE · ${timeLabel(m.at)}</div><div class="message-body">${bodyHTML(m.body)}</div></div>`).join('')||empty('No working notes yet','The skill teaches agents to record useful decisions, blockers, and findings here.');}
  ...
  if(view!=='settings')$('#channel-head .channel-actions')?.insertAdjacentHTML('afterbegin',button('search','Search','ghost small'));
}
function startPoll(){const id=++pollId;(async()=>{while(project&&id===pollId){try{const p=project;const changes=await api('changes?project='+p+'&after='+state.seq);if(id!==pollId)return;if(changes.seq!==state.seq)await refresh();connected=true;}catch(e){connected=false;toast('Connection interrupted. Retrying the local coordinator…',true);await new Promise(r=>setTimeout(r,4000));}}})();}
...
function connectHelp(a){const cmd=a?`vibeguild resume --project "${state.path}" --agent ${a.id}`:`vibeguild join --project "${state.path}" --handle builder --role implementer --provider claude`;modal(a?'Connect @'+a.handle:'Connect your agent','Run in the terminal where your agent can access Vibeguild.',`<div class="spacer"></div><pre class="join-code">${esc(cmd)}</pre><p class="muted">Install the agent-neutral skill, then tell the agent to load this project and create or resume its identity.</p><pre class="join-code">vibeguild install-skill --dest &lt;your-skills-folder&gt;</pre><p class="info-note">Use .agents/skills for Codex or .claude/skills for Claude Code. The coordinator stays running while your terminal watches for messages. A closed terminal cannot be woken by the skill alone.</p>`,async()=>{},'Done');}
document.addEventListener('click',async e=>{
 ...
 try{
  ...
  if(act==='agent-pane'){selectedAgentPane=el.dataset.pane;render();return await renderContent();}
  ...
  if(act==='new-agent')return modal('Add an agent profile','Identity persists when the terminal session ends.',field('Short name','handle')+field('Role','role','contributor')+field('Provider label','provider','codex'),async f=>{await command('register',{...Object.fromEntries(f),dormant:true});toast('Profile created. Open the agent to connect its terminal.');},'Create profile');
