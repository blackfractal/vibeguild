...
async function go(next){rememberScroll();if(next.startsWith('room:'))next=roomView(next.slice(5));if(next.startsWith('room:')||next.startsWith('agent:')){if(!tabs.includes(next))tabs.push(next);saveTabs();}view=next;oldest=null;render();await renderContent();await markCurrentRoomRead();}
...
async function renderContent(){
  ...
  if(view==='activity'){
    ...
    stream.innerHTML=`<div class="agent-sticky"><div class="agent-header">${avatar(a.id,'large')}<div><h2 style="color:${esc(a.color)}">${esc(a.handle)} ${state.config.lead_agent_id===a.id?'<span class="tag lead">LEAD</span>':''}</h2><p class="muted">${esc(a.role)} · ${esc(a.provider)}</p><div class="uuid">${a.id} ${button('copy-id',icon('copy'),'ghost small',`data-id="${a.id}" title="Copy UUID" aria-label="Copy UUID"`)}</div></div></div><div class="agent-controls">${button('rename-agent','Rename','small',`data-id="${a.id}"`)}${button('toggle-agent',icon(a.paused?'play':'pause')+' '+(a.paused?'Resume':'Pause at checkpoint'),'small',`data-id="${a.id}"`)}${state.config.lead_agent_id!==a.id?button('appoint-lead','Designate lead','small',`data-id="${a.id}"`):''}${button('agent-connect','Connect terminal','ghost small',`data-id="${a.id}"`)}<span class="tag">${esc(presence(a))}</span></div><div class="segment">${button('agent-pane','Direct chat',selectedAgentPane==='chat'?'active':'','data-pane="chat"')}${button('agent-pane','Working notes',selectedAgentPane==='notes'?'active':'','data-pane="notes"')}${button('agent-pane','Checkpoint & context',selectedAgentPane==='checkpoint'?'active':'','data-pane="checkpoint"')}</div></div><div id="agent-content"></div>`;
